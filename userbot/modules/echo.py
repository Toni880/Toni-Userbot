...
@toni_cmd(pattern="addecho(?: |$)(.*)")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await event.edit("`Balas pesan Pengguna untuk menggemakan pesannya`")
    tonievent = await event.edit("`Tambahkan Echo ke pengguna...`")
    user, rank = await get_user_from_event(event, tonievent, nogroup=True)
    if not user:
        return
...
async def echo(event):
    ...
    if is_echo(chat_id, user_id):
        try:
            ...
            await edit_delete(tonievent, f"**Error:**\n`{str(e)}`")
        else:

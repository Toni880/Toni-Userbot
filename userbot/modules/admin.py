...
async def promote(event):
    ...
    try:
        ...
        await edit_delete(tonicevent, "`Berhasil Menambahkan Pengguna Ini Sebagai Admin!`")

    # If Telethon spit BadRequestError, assume
    # we don't have Promote permission
    except BadRequestError:
        return await tonicevent.edit(NO_PERM)
...
async def demote(event):
    ...
    tonicevent = await edit_or_reply(event, "`Sedang Melepas Admin...`")
    ...
    # Assume we don't have permission to demote
        ...
        return await tonicevent.edit(NO_PERM)
    await edit_delete(tonicevent, "`Berhasil Melepas Pengguna Ini Sebagai Admin!`")
...
async def ban(bon):
    ...
    toni = await edit_or_reply(bon, "`Melakukan Banned!`")
...
async def nothanos(unbon):
    ...
    toni = await edit_or_reply(unbon, "`Sedang Melakukan Unban...`")
    ...
    try:
        ...
        await edit_delete(toni, "```Berhasil Melepas Ban Pengguna!```")
        ...
        await edit_delete(toni, "`Sepertinya Terjadi Kesalahan!`")


@toni_cmd(pattern="mute(?: |$)(.*)")
...
async def spider(spdr):
    ...
    toni = await edit_or_reply(spdr, "`Sedang melakukan Mute...`")
    ...
    if user.id == self_user.id:
        return await edit_or_reply(toni,
                                   "`Tangan Terlalu Pendek, Tidak Bisa Membisukan Diri Sendiri...\n(ヘ･_･)ヘ┳━┳`"
                                   )

    # If everything goes well, do announcing and mute
    await toni.edit("`Telah Dibisukan!`")
    if mute(spdr.chat_id, user.id) is False:
        return await toni.edit("`Pengguna Sudah Dibisukan!`")
    else:
        try:
            ...
            if reason:
                await toni.edit(f"**Pengguna Telah Dibisukan!**\n**Alasan:** `{reason}`")
            else:
                await toni.edit("`Telah Dibisukan!`")
            ...
            return await edit_delete(toni, "`Terjadi Kesalahan!`")


@toni_cmd(pattern="unmute(?: |$)(.*)")
...
async def unmoot(unmot):
    ...
    toni = await edit_or_reply(unmot, "```Melakukan Unmute...```")
    ...
    if unmute(unmot.chat_id, user.id) is False:
        ...
        try:
            ...
            await edit_delete(toni, "```Berhasil Melakukan Unmute! Pengguna Sudah Tidak Dibisukan```")
        except UserIdInvalidError:
            return await edit_delete(toni, "`Terjadi Kesalahan!`")
...
async def ungmoot(un_gmute):
    ...
    toni = await edit_or_reply(un_gmute, "`Sedang melakukan membuka Global Mute...`")
    ...
    await toni.edit("```Membuka Global Mute Pengguna...```")

    if ungmute(user.id) is False:
        await toni.edit("`Pengguna Sedang Tidak Di Gmute!`")
    else:
...
async def gspider(gspdr):
    ...
    toni = await edit_or_reply(gspdr, "`Processing...`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return

    # If pass, inform and start gmuting
    await toni.edit("`Pengguna Berhasil Dibisukan!`")
    if gmute(user.id) is False:
        await edit_delete(gspdr, "`Kesalahan! Pengguna Sudah Dibisukan.`")
    else:
        if reason:
            await toni.edit(f"**Dibisukan Secara Global!**\n**Alasan:** `{reason}`")
        else:

...
from userbot.utils import edit_delete, edit_or_reply, toni_cmd
...
async def get_call(toni):
    toni = await toni.client(getchat(toni.chat_id))
    await toni.client(getvc(toni.full_chat.call, limit=1))
    return hehe.call
...
@toni_cmd(pattern="vcinvite")
async def _(toni):
    await edit_or_reply(toni, "`Sedang Menginvite Member...`")
    users = []
    z = 0
    async for x in toni.client.iter_participants(toni.chat_id):
        if not x.bot:
            users.append(x.id)
    hmm = list(user_list(users, 6))
    for p in hmm:
        try:
            await toni.client(invitetovc(call=await get_call(toni), users=p))
            z += 6
        except BaseException:
            pass
    await edit_or_reply(toni, f"`Menginvite {z} Member`")


@toni_cmd(pattern="vctitle(?: |$)(.*)")
